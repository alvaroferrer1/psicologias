...
export function PatientEditSheet({ patient }: Props) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (res.success) {
      toast({ type: "success", title: "Paciente actualizado" });
      setIsOpen(false);
      window.location.reload();
    } else {
      toast({ type: "error", title: res.error || "No se pudo guardar" });
    }
    setIsSaving(false);
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm">
          <div className="w-full max-w-3xl overflow-hidden rounded-[24px] bg-white shadow-2xl">
            <div className="border-b border-slate-100 bg-slate-50 p-6">
              <h3 className="text-lg font-bold text-secondary-text">{"Editar paciente"}</h3>
            </div>

            <form onSubmit={handleSubmit} className="space-y-4 p-6">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="md:col-span-2">
                  <label className="mb-1 block text-[13px] font-bold text-slate-700">{"Nombre"}</label>
                ...
                <div>
                  <label className="mb-1 block text-[13px] font-bold text-slate-700">{"Tipo de paciente"}</label>
                ...
                <div>
                  <label className="mb-1 block text-[13px] font-bold text-slate-700">{"Teléfono apoderado"}</label>
                  <input type="text" className="inp" value={formData.guardianPhone} onChange={(e) => setFormData((prev) => ({ ...prev, guardianPhone: e.target.value }))} />
                </div>
                <div>
                  <label className="mb-1 block text-[13px] font-bold text-slate-700">{"Email apoderado"}</label>
